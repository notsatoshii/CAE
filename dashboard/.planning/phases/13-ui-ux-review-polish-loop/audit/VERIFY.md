...
| Memory tree leaf count | `/api/memory/tree` | 394 | AUTH-REQUIRED | ⚠️ AUTH-DEFERRED |
| Chat unread count (WR-01 static analysis) | `static-analysis` | BUG CONFIRMED: 4 randomUUID() calls in stream (expected 1). lastSeenMsgId overwritten → unread always 0 | N/A | ❌ CODE-BUG |
